...
const startSock = async () => {
  ...
  const chatList = [];
  const pre = "#";
  const botId = "919532122374@s.whatsapp.net";
  let i = 0;
  ...
  setInterval(async () => {
    while (allMsgArray.length > 0 && false) {
      //await delay(300);
      try {
              ...
              case "enable":
                if (
                  ...
                  if (
                    ...
                    const val = await DbOperation.enableCmd(chatId, msgData);
                    if (val) {
                      await sock.sendMessage(
                        chatId,
              ...
              case "disable":
                if (
                  ...
                  if (
                    ...
                    const val = await DbOperation.disableCmd(chatId, msgData);
                    if (val) {
                      await sock.sendMessage(
                        chatId,
